...
def allowed_in_scenario(day_name: str, start_hour: int, L: int, scenario: str, is_whole_class: bool) -> bool:
    end_hour = start_hour + L

    if scenario == "baseline":
        return True

    elif scenario == "mon_fri_9_5":
        # Only restrict whole class
        if is_whole_class:
            return end_hour <= 17
        return True

    elif scenario == "no_friday_afternoon":
        if day_name == "Friday" and end_hour > 12:
            return False
        return True
...
def main(time_limit_sec: int = 900, N_EVENTS: int | None = 2000,
    ...
    type_to_rooms = rm.groupby(
        "room_type").indices  # First, group the room_type into a "list of row indices for each type,
    type_capacity = {t: len(idx_list) for t, idx_list in
                     type_to_rooms.items()}  # then calculate the length of each group to get the room count.
    ...
    # Only run the first two weeks first, delete the line below to run full data
    ...
    # Split events: need-room / no-room
    ...
    print("Stage A: schedule only need-room events")
    ...
    # (H1) + (H2)
    # For each need-room event: We only allow feasible starts (t+L<=NUM_SLOTS) and enforce exactly-one
    for i in E_room:
        L = int(ev.at[i, "L"])
        is_whole_class = (ev.at[i, "whole_class"] == "TRUE")  # ← add this line
        opts = []
        for d in range(len(DAYS)):
            for t in range(NUM_SLOTS):
                # (H2) feasibility of start time for duration L
                start_hour = SLOT_START_HOURS[t]
                day_name = DAYS[d]

                if t + L <= NUM_SLOTS and allowed_in_scenario(day_name, start_hour, L, scenario, is_whole_class):
        ...
        if not opts:
            raise RuntimeError(
                f"[StageA] event_id={ev.at[i, 'event_id']} L={L} > NUM_SLOTS={NUM_SLOTS} Or no available starting point")
    ...
    solverA.parameters.stop_after_first_solution = bool(stop_after_first_solution)
    ...
    print("Stage B: schedule no-room events")
    ...
    for i in ev.index:
        ...
        if not opts:
            raise RuntimeError(
                f"[StageB] event_id={ev.at[i, 'event_id']} L={L} > NUM_SLOTS={NUM_SLOTS} or no available starting point")

        # (H1) exactly one template
        modelB.Add(sum(startB[(i, d, t)] for (d, t) in opts) == 1)

        # (H5) Pattern Constraint: Same Time Template Across Weeks
        # Fix the template for need-room events to Stage A result
        if i in fixed_dt:
    ...
    solverB.parameters.stop_after_first_solution = bool(stop_after_first_solution)
    ...
    sol_path = OUTDIR / f"step1_solution_{scenario}.csv"
    sol.to_csv(sol_path, index=False)
    print(f"Complete timetable ({scenario}):", sol_path)
