...
def main():
    ...
    if all_results:
        ...
        # Create a copy first to avoid modifying the original numeric table
        display_df = result_df.copy()

        # objective keep 2 decimal places
        display_df["objective"] = display_df["objective"].map(lambda x: f"{x:.2f}")

        # Regular percentage columns: already like 1.38 / 22.66, just add %
        for col in pct_cols:
            display_df[col] = display_df[col].map(lambda x: f"{x:.2f}%")

        # Utilisation columns: originally like 1.0 / 0.55, multiply by 100 first then add %
        for col in util_pct_cols:
            display_df[col] = display_df[col].map(lambda x: f"{x * 100:.2f}%")

        # Other columns keep 2 decimal places
        display_df["avg_events_per_used_timeslot"] = display_df["avg_events_per_used_timeslot"].map(
            lambda x: f"{x:.2f}")
