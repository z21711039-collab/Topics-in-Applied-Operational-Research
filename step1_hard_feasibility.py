...
def allowed_in_scenario(day_name: str, start_hour: int, L: int, scenario: str, is_whole_class: bool) -> bool:
    ...
    if scenario == "baseline":
        ...
        # 限制所有 event（不区分 whole class）
        if day_name == "Friday" and start_hour >= 12:
            return False
        return True
...
# Clean events
    ...
    for i in E_room:
        L = int(ev.at[i, "L"])
        is_whole_class = (ev.at[i, "whole_class"] == "TRUE")   # ← 加这一行
        opts = []
    ...
    for i in ev.index:
        ...
        if not opts:
            raise RuntimeError(f"[StageB] event_id={ev.at[i,'event_id']} L={L} > NUM_SLOTS={NUM_SLOTS} or no available starting point")
